...
void main() {
    ...
    for (int i = 0; i < 50; i++) {
        ...
        float influence = getMetaBallValue(iMetaBalls[i].xy, iMetaBalls[i].z, coord);
        colorAccum += iColors[i] * influence;
        totalInfluence += influence;
    }
    
    // Add cursor ball influence
    float cursorInfluence = getMetaBallValue(mouseW, iCursorBallSize, coord);
    colorAccum += iCursorColor * cursorInfluence;
    totalInfluence += cursorInfluence;
    
    // Normalize color by total influence
    vec3 cFinal = totalInfluence > 0.0 ? colorAccum / totalInfluence : vec3(0.0);
    
    // Create smooth edge
    float f = smoothstep(-1.0, 1.0, (totalInfluence - 1.3) / min(1.0, fwidth(totalInfluence)));
...
const MetaBalls = ({
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < 50; i++) {
            ...
            const [r, g, b] = parseHexColor(colors[colorIndex])
            colorUniform.push(new Vec3(r, g, b))
        }
        ...
        // Listen on window to avoid scroll interference
        ...
        function update(t: number) {
            ...
            // Reduce update frequency during scroll (every other frame)
            if (isScrolling) {
                frameCount++
                if (frameCount % 2 !== 0) {
